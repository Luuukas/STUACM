...
function getInfo(url, cnt, tot, infoArray) {
    ...
    var p = new Promise(function (resolve, reject) {
        superagent.get(url + cnt).end(function (err, ares) {
            ...
            var submissionsNum = atable.length;    // how many submissions in this page
            ...
            for (var j = 1; j <= submissionsNum; j++) {
                ...
                var verdict = $($($(atr[5]).children("span")[0]).children("span")[0]).text();    // state of the submission
                var time = $(atr[1]).text();
                // var match_res = problemurl.match(/https:\/\/codeforces.com\/contest\/(.*)\/problem\/(.*)/g)
                // console.log(match_res)
                if (infoArray[problemurl] == undefined || verdict=="Accepted") {
